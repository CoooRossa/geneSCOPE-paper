...
Optional:
  ...
  --svg_I            Moran's I threshold (optional; pass through to runner)
  --svg_topk         TopK Moran's I genes (optional; pass through to runner)
  --n_permutation    Permutations for Global_L p-value (pass through)
  --permute_ratio    permute_ratio/percent used in permutation (pass through)
  --fdr_cutoff       FDR cutoff (pass through)
  --l_cutoff         Effect-size cutoff on L (pass through)
  --indep            BH indep=1/0 (pass through)
  --modules_n        Force module count >=2 (pass through)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --svg_topk=*)
      SVG_TOPK="${1#*=}"; shift;;
    --n_permutation)
      N_PERMUTATION="$2"; shift 2;;
    --n_permutation=*)
      N_PERMUTATION="${1#*=}"; shift;;
    --permute_ratio)
      PERMUTE_RATIO="$2"; shift 2;;
    --permute_ratio=*)
      PERMUTE_RATIO="${1#*=}"; shift;;
    --fdr_cutoff)
      FDR_CUTOFF="$2"; shift 2;;
    --fdr_cutoff=*)
      FDR_CUTOFF="${1#*=}"; shift;;
    --l_cutoff)
      L_CUTOFF="$2"; shift 2;;
...
write_run_args() {
  ...
  if ! DATA_DIR="${DATA_DIR}" OUTDIR="${OUTDIR}" COORD_FILE="${COORD_FILE}" \
      ...
      GRID_UM="${GRID_UM}" MODULES_NMAX="${MODULES_NMAX}" \
      SVG_I="${SVG_I}" SVG_TOPK="${SVG_TOPK}" N_PERMUTATION="${N_PERMUTATION}" PERMUTE_RATIO="${PERMUTE_RATIO}" \
      FDR_CUTOFF="${FDR_CUTOFF}" L_CUTOFF="${L_CUTOFF}" INDEP="${INDEP}" MODULES_N="${MODULES_N}" \
      EXTRA_ARGS_JSON="${EXTRA_ARGS_JSON}" RUN_ARGS_JSON="${out}" \
...
payload = {
    ...
    "svg_I": os.environ.get("SVG_I") or None,
    "svg_topk": int(os.environ.get("SVG_TOPK", "0")) if os.environ.get("SVG_TOPK") else None,
    "n_permutation": int(os.environ.get("N_PERMUTATION", "0")) if os.environ.get("N_PERMUTATION") else None,
    "permute_ratio": float(os.environ.get("PERMUTE_RATIO", "0")) if os.environ.get("PERMUTE_RATIO") else None,
    "fdr_cutoff": float(os.environ.get("FDR_CUTOFF", "0")) if os.environ.get("FDR_CUTOFF") else None,
    "l_cutoff": float(os.environ.get("L_CUTOFF", "0")) if os.environ.get("L_CUTOFF") else None,
    "indep": int(os.environ.get("INDEP", "0")) if os.environ.get("INDEP") else None,
    "modules_n": int(os.environ.get("MODULES_N", "0")) if os.environ.get("MODULES_N") else None,
...
if [[ -n "${TIME_BIN}" && -x "${TIME_BIN}" ]]; then
  ...
  SEAGAL_PASSTHRU_ARGS=()
  if [[ -n "${SVG_I}" ]]; then
    SEAGAL_PASSTHRU_ARGS+=(--svg_I "${SVG_I}")
  fi
  if [[ -n "${SVG_TOPK}" ]]; then
    SEAGAL_PASSTHRU_ARGS+=(--svg_topk "${SVG_TOPK}")
  fi
  if [[ -n "${N_PERMUTATION}" ]]; then
    SEAGAL_PASSTHRU_ARGS+=(--n_permutation "${N_PERMUTATION}")
  fi
  if [[ -n "${PERMUTE_RATIO}" ]]; then
    SEAGAL_PASSTHRU_ARGS+=(--permute_ratio "${PERMUTE_RATIO}")
  fi
  if [[ -n "${FDR_CUTOFF}" ]]; then
    SEAGAL_PASSTHRU_ARGS+=(--fdr_cutoff "${FDR_CUTOFF}")
  fi
  if [[ -n "${L_CUTOFF}" ]]; then
    SEAGAL_PASSTHRU_ARGS+=(--l_cutoff "${L_CUTOFF}")
  fi
  if [[ -n "${INDEP}" ]]; then
    SEAGAL_PASSTHRU_ARGS+=(--indep "${INDEP}")
  fi
  if [[ -n "${MODULES_N}" ]]; then
    SEAGAL_PASSTHRU_ARGS+=(--modules_n "${MODULES_N}")
  fi
  ...
  if [[ ! -s "${repdir}/all_edges.tsv" ]]; then
    echo "[ERROR] Missing or empty all_edges.tsv: ${repdir}/all_edges.tsv" >&2
    log "[ERROR] Missing or empty all_edges.tsv: ${repdir}/all_edges.tsv"
    exit 1
  fi
