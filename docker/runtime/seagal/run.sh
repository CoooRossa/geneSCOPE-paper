...
if [[ "${SAMPLE_SEC}" == "0" ]]; then
  log "[INFO] stats monitoring disabled (--sample_sec=0); ${OUTDIR}/stats.tsv will only contain the header"
elif [[ -f /opt/app/monitor_cgroup.sh ]]; then
  ...
  log "[WARN] monitor_cgroup.sh not found at /opt/app/monitor_cgroup.sh; stats monitoring disabled"
fi
