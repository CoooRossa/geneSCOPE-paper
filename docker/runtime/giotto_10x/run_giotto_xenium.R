...
extract_complete_edges <- function(spat_cor_obj, gene_set) {
  ...
  data.table::setorder(weights_dt, gene_a, gene_b)
  weights_dt
}
...
select_corr_genes <- function(expr_mat, n_corr_genes, method = c("mean", "variance")) {
  ...
  if (inherits(expr_mat, "dgCMatrix")) {
    ...
    if (method == "mean") {
      ...
      rsq <- tabulate(expr_mat@i + 1L, nbins = nrow(expr_mat), weights = expr_mat@x * expr_mat@x)
      m2 <- rsq / n_obs
      score <- m2 - mu ^ 2
      score[score < 0] <- 0
    }
...
if (emit_edge_stats != 0L) {
  eprint("[WARN] --emit_edge_stats ignored (edge export uses constant p_value/fdr=1.0).")
}
...
edges_base <- extract_complete_edges(spat_cor_obj, gene_set = selected_genes)
edges_outfile <- "gene_graph_edges.tsv"
edges_dt <- data.table::as.data.table(edges_base)
if (!all(c("gene_a", "gene_b", "weight") %in% names(edges_dt))) {
...
edges_dt <- edges_dt[is.finite(weight)]
setcolorder(edges_dt, c("gene_a", "gene_b", "weight", "p_value", "fdr", "n_obs", "edge_type"))
if (!identical(names(edges_dt), c("gene_a", "gene_b", "weight", "p_value", "fdr", "n_obs", "edge_type"))) {
  stop("edges export schema mismatch after setcolorder.")
}

fwrite(edges_dt, file.path(opt$outdir, edges_outfile), sep = "\t")
