...
def run_seagal_official_api(
    ...
    *,
    ...
    # NOTE: This runner computes full-gene `edges_all.tsv` in `main()` by splitting the gene
    # universe into blocks and repeatedly calling SEAGAL's own `spatial_association()` on each block
    # (and block pairs). This avoids allocating an n_obs×n_pairs dense matrix for all genes at once.
    meta["edges_all_algorithm"] = "seagal_chunked_spatial_association"
    n_full_genes = 0
    meta["n_assoc_genes_full"] = int(n_full_genes)
    ...
    # ---- spatial_association (FULL genes; edges_all.tsv) ----
    if n_full_genes >= 2:
        spatial_association = getattr(api, "spatial_association", None)
        coexpr = None
        if callable(spatial_association):
            assoc_exc_root: Optional[BaseException] = None
            assoc_errors: Dict[str, str] = {}
            for mode, args in _spatial_association_call_modes(
                spatial_association,
                sg_obj=sg,
                grouped_obj=grouped_result,
                adata_obj=adata,
            ):
                try:
                    ret = _call_api(
                    ...
                    if coexpr is None:
                        coexpr = _find_df_with_cols(
                            getattr(sg, "co_expression", None), ["gene_1", "gene_2", "L"]
                        )
                    if coexpr is None:
                        coexpr = _find_df_with_cols(getattr(sg, "adata", None), ["gene_1", "gene_2", "L"])
                    if coexpr is None:
                        raise RuntimeError(
                            "Cannot locate SEAGAL SAG/co_expression table after spatial_association (full)"
                        )
                    sg.co_expression = coexpr
                    meta["spatial_association_full_mode"] = mode
                    chain_ok("spatial_association")
                    break
                except Exception as exc:
                    if assoc_exc_root is None:
                        assoc_exc_root = exc
                    assoc_errors[mode] = f"{type(exc).__name__}: {exc}"
                    continue
            else:
                chain_miss("spatial_association")
                meta["spatial_association_full_errors"] = assoc_errors
                raise RuntimeError(
                    "SEAGAL spatial_association (full genes) failed. "
                    f"entrypoint={entrypoint} errors={assoc_errors}"
                ) from assoc_exc_root
        else:
            chain_miss("spatial_association")
            meta["spatial_association_skipped_reason"] = "not_found"
            coexpr = _find_df_with_cols(grouped_result, ["gene_1", "gene_2", "L"])
            if coexpr is None:
                coexpr = _find_df_with_cols(getattr(sg, "co_expression", None), ["gene_1", "gene_2", "L"])
            if coexpr is None:
                coexpr = _find_df_with_cols(getattr(sg, "adata", None), ["gene_1", "gene_2", "L"])
            if coexpr is None:
                raise AttributeError(
                    f"SEAGAL api missing spatial_association and no SAG table found. entrypoint={entrypoint}"
                )
            sg.co_expression = coexpr

        if isinstance(getattr(sg, "co_expression", None), pd.DataFrame):
            coexpr_full = sg.co_expression.copy()
        meta["full_edges_rows"] = int(coexpr_full.shape[0]) if isinstance(coexpr_full, pd.DataFrame) else 0
    else:
        sg.co_expression = pd.DataFrame(
            columns=[
                "gene_1",
...
def _standardize_and_clip_inplace(X: np.ndarray, *, clip: float = 10.0) -> np.ndarray:
...
def compute_global_L_weight_matrix_chunked(
    adata: "anndata.AnnData",
    *,
    var_names: List[str],
) -> np.ndarray:
    """
    Compute SEAGAL Global_L for all gene pairs without allocating n_obs×n_pairs matrices.

    Upstream SEAGAL constructs (n_obs, n_pairs) dense matrices (features_1_X/features_2_X).
    This implementation is mathematically equivalent for the Global_L association statistic:

      L(i,j) = z_j^T (C^T C) z_i / denom
      where z_g is StandardScaler(expr_g) clipped to [-10,10],
      C = adata.obsp['spatial_connectivities'], denom = 1^T (C^T C) 1.
    """
    from scipy import sparse

    if adata is None:
        raise ValueError("compute_global_L_weight_matrix_chunked: adata is None")

    if "spatial_connectivities" not in getattr(adata, "obsp", {}):
        raise ValueError("Missing adata.obsp['spatial_connectivities']; spatial_neighbors not run?")

    c = adata.obsp["spatial_connectivities"]
    if sparse.issparse(c):
        C = c.tocsr().astype(np.float64)
    else:
        C = sparse.csr_matrix(np.asarray(c, dtype=np.float64))

    n_obs = int(C.shape[0])
    if n_obs <= 1:
        raise ValueError(f"Need >=2 observations for Global_L (n_obs={n_obs}).")

    var_index = pd.Index(adata.var_names.astype(str))
    idx = var_index.get_indexer([str(v) for v in var_names])
    if (idx < 0).any():
        missing = [var_names[i] for i, j in enumerate(idx.tolist()) if j < 0]
        raise ValueError(f"var_names missing in adata.var_names: {missing[:10]}")

    X = adata.X[:, idx]
    if sparse.issparse(X):
        X = X.toarray()
    X = _standardize_and_clip_inplace(np.asarray(X, dtype=np.float64))

    ones = np.ones(n_obs, dtype=np.float64)
    w = C @ ones
    denom = float(np.dot(w, w))
    if not np.isfinite(denom) or denom == 0.0:
        raise ValueError(f"Invalid denom for Global_L: denom={denom}")

    eprint(f"[INFO] Global_L(chunked) n_obs={n_obs} n_genes={X.shape[1]} denom={denom:.6g}")

    # L = X^T (C^T C) X / denom = (C X)^T (C X) / denom
    CX = np.asarray(C @ X, dtype=np.float64, order="C")
    L = (CX.T @ CX) / denom
...
def write_edges_all_tsv_from_weight_matrix(
    L: np.ndarray,
    gene_names: List[str],
    out_path: str,
    *,
    ...
    gene_arr = np.asarray([str(g) for g in gene_names], dtype=object)
    n_genes = int(gene_arr.shape[0])
    if n_genes < 2:
    ...
    if L.shape != (n_genes, n_genes):
        raise ValueError(f"L shape {L.shape} does not match gene count {n_genes}.")

    with open(out_path, "w", encoding="utf-8") as f:
        f.write("gene_a\tgene_b\tweight\tp_value\tfdr\tn_obs\tedge_type\n")

    total_edges = n_genes * (n_genes - 1) // 2
    written = 0

    for j0 in range(1, n_genes, int(block_cols)):
        j1 = min(n_genes, j0 + int(block_cols))
        i_parts: List[np.ndarray] = []
        j_parts: List[np.ndarray] = []
        for j in range(j0, j1):
            if j <= 0:
                continue
            i_parts.append(np.arange(0, j, dtype=np.int32))
            j_parts.append(np.full(j, j, dtype=np.int32))
        if not i_parts:
            continue
        i_idx = np.concatenate(i_parts)
        j_idx = np.concatenate(j_parts)

        weights = np.asarray(L[i_idx, j_idx], dtype=np.float64)
        ...
        df = pd.DataFrame(
            {
                "gene_a": gene_arr[i_idx],
                "gene_b": gene_arr[j_idx],
                "weight": weights,
        ...
        written += int(df.shape[0])
        eprint(f"[INFO] edges_all.tsv progress: {written}/{total_edges}")
...
def _auto_seagal_edges_block_genes(
    *,
    n_obs: int,
    n_genes: int,
    ...
    """
    Choose a gene block size so that a SEAGAL (A+B) run is likely to fit in memory.

    SEAGAL Global_L allocates roughly:
      features_1_X + features_2_X + their standardized copies
    ~= 4 * n_obs * n_pairs * 8 bytes.
    For a block-pair run, n_pairs ~= m*(m-1)/2 where m ~= 2*block_genes.
    """
    if n_genes <= 0:
        return 0
    if n_genes <= 2:
    ...
    mem = float(memory_budget_gib)
    if not np.isfinite(mem) or mem <= 0:
        return min(256, int(n_genes))

    budget_bytes = mem * float(1024**3)
    denom = 16.0 * float(n_obs)  # see docstring
    m_max = int(np.floor(np.sqrt(max(0.0, budget_bytes / denom))))
    block = max(2, m_max // 2)
    return int(min(block, int(n_genes)))
...
def write_edges_all_tsv_seagal_chunked(
    sg: object,
    *,
    full_genes: List[str],
    out_path: str,
    ...
    blocks = _chunk_list([str(g) for g in full_genes], int(block_genes))
    total_pairs = int(len(full_genes) * (len(full_genes) - 1) // 2)

    with open(out_path, "w", encoding="utf-8") as f:
        f.write("gene_a\tgene_b\tweight\tp_value\tfdr\tn_obs\tedge_type\n")

    written = 0
    for bi, genes_i in enumerate(blocks):
        eprint(f"[INFO] edges_all: block {bi+1}/{len(blocks)} within size={len(genes_i)}")
        spatial_association(
            sg,
            grouped_only=False,
            use_pattern_genes=False,
        ...
        df_within = getattr(sg, "co_expression", pd.DataFrame())
        written += _append_edges_all_from_seagal_coexpr(
            df_within, out_path, n_obs=int(n_obs), edge_type="seagal_global_L"
        )
        try:
            setattr(sg, "co_expression", None)
        except Exception:
            pass
        eprint(f"[INFO] edges_all: progress {written}/{total_pairs}")

        set_i = set(genes_i)
        for bj in range(bi + 1, len(blocks)):
            genes_j = blocks[bj]
            eprint(
                f"[INFO] edges_all: block-pair ({bi+1},{bj+1}) union_size={len(genes_i)+len(genes_j)}"
            )
            spatial_association(
                sg,
                grouped_only=False,
                use_pattern_genes=False,
                genes=genes_i + genes_j,
                n_permutation=int(n_permutation),
                permute_ratio=float(permute_ratio),
                FDR_cutoff=float(fdr_cutoff),
                L_cutoff=float(l_cutoff),
            ...
            df_pair = getattr(sg, "co_expression", pd.DataFrame())
            if isinstance(df_pair, pd.DataFrame) and not df_pair.empty:
                set_j = set(genes_j)
                mask = df_pair["gene_1"].astype(str).isin(set_i) & df_pair["gene_2"].astype(str).isin(set_j)
                df_cross = df_pair.loc[mask].copy()
                written += _append_edges_all_from_seagal_coexpr(
                    df_cross, out_path, n_obs=int(n_obs), edge_type="seagal_global_L"
                )
            try:
                setattr(sg, "co_expression", None)
            except Exception:
    ...
    if written != total_pairs:
        eprint(f"[WARN] edges_all rows mismatch: wrote={written} expected={total_pairs}")
    return int(written)
...
def main() -> None:
    ...
    # ---- Full-gene edges_all via chunked SEAGAL spatial_association ----
    ...
    edges_block_genes = _safe_int(extra.get("seagal_edges_block_genes", 0), 0)
    edges_mem_budget_gib = _safe_float(
        extra.get("seagal_edges_memory_budget_gib", min(64.0, _available_memory_gib() * 0.6)),
        min(64.0, _available_memory_gib() * 0.6),
    )
    if edges_block_genes <= 0:
        edges_block_genes = _auto_seagal_edges_block_genes(
            n_obs=int(n_obs_post),
            n_genes=int(n_vars_post),
            memory_budget_gib=float(edges_mem_budget_gib),
        )
    if edges_block_genes < 2:
        edges_block_genes = min(256, int(n_vars_post))

    edges_n_permutation = _safe_int(extra.get("seagal_edges_n_permutation", 0), 0)
    eprint(
        "[INFO] edges_all config: n_genes=%d block_genes=%d mem_budget_gib=%.2f n_permutation=%d",
        int(n_vars_post),
        int(edges_block_genes),
        float(edges_mem_budget_gib),
        int(edges_n_permutation),
    )
    edges_full_n = write_edges_all_tsv_seagal_chunked(
        sg,
        full_genes=full_genes,
        out_path=edges_all_path,
        n_obs=int(n_obs_post),
        block_genes=int(edges_block_genes),
        n_permutation=int(edges_n_permutation),
        permute_ratio=float(permute_ratio),
        fdr_cutoff=float(fdr_cutoff),
        l_cutoff=float(l_cutoff),
        indep=bool(indep),
    ...
    edges_all_pattern_path = os.path.join(outdir, "edges_all_pattern.tsv")
    edges_pattern_n = 0
    ...
    if pattern_genes and len(pattern_genes) >= 2:
        ...
        with open(edges_all_pattern_path, "w", encoding="utf-8") as f:
            f.write("gene_a\tgene_b\tweight\tp_value\tfdr\tn_obs\tedge_type\n")
        if isinstance(df_pat, pd.DataFrame) and not df_pat.empty:
            edges_pattern_n = _append_edges_all_from_seagal_coexpr(
                df_pat, edges_all_pattern_path, n_obs=int(n_obs_post), edge_type="seagal_global_L_pattern"
            )
    ...
    stats = {
        ...
        "edges_full_n": int(edges_full_n),
        "edges_pattern_n": int(edges_pattern_n),
        "seagal_edges_block_genes": int(edges_block_genes),
        "seagal_edges_memory_budget_gib": float(edges_mem_budget_gib),
        "seagal_edges_n_permutation": int(edges_n_permutation),
    ...
    try:
        ...
        meta_payload = {
            ...
            "counts": {
                ...
                "sag_pairs_n": int(edges_full_n),
                "sag_pairs_full_n": int(edges_full_n),
            ...
            "outputs": {
                ...
                "edges_all_pattern_tsv": "edges_all_pattern.tsv",
    ...
    eprint(f"[DONE] wrote: {edges_all_pattern_path}")
