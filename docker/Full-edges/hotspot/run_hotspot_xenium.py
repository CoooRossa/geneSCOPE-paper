...
def _format_edge_fdr(edge_fdr: float) -> str:
    s = f"{float(edge_fdr):.6g}"
    return s.split(".")[0] if s.endswith(".0") else s


def _write_edge_matrix(edges_df: pd.DataFrame, outdir: str, prefix: str) -> bool:
    if edges_df.empty:
        return False
    try:
    ...
    genes = sorted(set(edges_df["gene_a"]).union(edges_df["gene_b"]))
    gene_idx = {gene: i for i, gene in enumerate(genes)}
    i = edges_df["gene_a"].map(gene_idx).to_numpy()
    j = edges_df["gene_b"].map(gene_idx).to_numpy()
    w = pd.to_numeric(edges_df["weight"], errors="coerce").to_numpy()
    mask = np.isfinite(i) & np.isfinite(j) & np.isfinite(w)
    i = i[mask]
    j = j[mask]
    w = w[mask]
    if w.size == 0:
    ...
    mmwrite(os.path.join(outdir, f"{prefix}_matrix.mtx"), mat)
    pd.DataFrame({"gene": genes, "idx": np.arange(1, len(genes) + 1)}).to_csv(
        os.path.join(outdir, f"{prefix}_genes.tsv"),
        sep="\t",
        index=False,
    )
    return True
...
def emit_hotspot_edges(
    ...
    edge_fdr_str = _format_edge_fdr(edge_fdr)
    edge_top_limit = max(0, int(top_n))
    edges_top_path = os.path.join(outdir, f"edges_fdr{edge_fdr_str}_top{edge_top_limit}.tsv")
    empty_cols = ["gene_a", "gene_b", "weight", "p_value", "fdr", "n_obs", "edge_type"]
    ...
    if w_df is None and z_df is None:
        ...
        empty.to_csv(edges_all_path, sep="\t", index=False)
        empty.to_csv(edges_top_path, sep="\t", index=False)
        return {
            "edges_total_n": 0,
            "edges_sig_n": 0,
            "edges_top_n": 0,
    ...
    weight_type = "local_corr"
    if c_df is None and local_corr is None:
        weight_type = "local_corr_z"
    ...
    if z_df is None:
        raise RuntimeError(
            "Hotspot local_correlation_z missing; cannot compute official p/FDR for edges."
        )
    ...
    p_vec, fdr_vec, _ = _hotspot_official_localcorr_p_fdr(z_df, genes)

    n = len(genes)
    tri = np.triu_indices(n, k=1)
    w_vals = w_mat[tri]
    mask = np.isfinite(w_vals) & np.isfinite(p_vec) & np.isfinite(fdr_vec)
    ...
    edges_all = pd.DataFrame(
        {
            ...
            "n_obs": int(n_obs),
            "edge_type": "local_corr",
        }
    )

    gene_min = edges_all[["gene_a", "gene_b"]].min(axis=1)
    gene_max = edges_all[["gene_a", "gene_b"]].max(axis=1)
    edges_all["gene_a"] = gene_min
    edges_all["gene_b"] = gene_max
    edges_all = edges_all.drop_duplicates(subset=["gene_a", "gene_b"])

    edges_all.to_csv(edges_all_path, sep="\t", index=False)

    edges_sig = edges_all[
        np.isfinite(edges_all["fdr"]) & (edges_all["fdr"] < float(edge_fdr))
    ].copy()
    if not edges_sig.empty:
        edges_sig = edges_sig.sort_values("weight", ascending=False)
    edges_top = edges_sig.head(edge_top_limit)
    edges_top.to_csv(edges_top_path, sep="\t", index=False)

    _write_edge_matrix(edges_top, outdir, f"edges_fdr{edge_fdr_str}_top{edge_top_limit}")

    return {
        "edges_total_n": int(edges_all.shape[0]),
        "edges_sig_n": int(edges_sig.shape[0]),
        "edges_top_n": int(edges_top.shape[0]),
        "edges_weight_type": weight_type,
        "edges_pvalue_method": HOTSPOT_LOCALCORR_PVALUE_METHOD,
        "edges_fdr_threshold": float(edge_fdr),
    }
...
def _write_empty_gene_graph_edges(outdir: str) -> None:
    cols = ["gene_a", "gene_b", "weight", "p_value", "fdr", "edge_source"]
...
def emit_hotspot_gene_graph_edges(
    ...
    edge_source = "hotspot_local_correlation"
    ...
    if w_df.shape[0] == w_df.shape[1]:
        ...
        edges_sig = edges_all[
            (edges_all["weight"] > 0)
            & np.isfinite(edges_all["fdr"])
            & (edges_all["fdr"] <= float(fdr_threshold))
        ].copy()
...
def run_hotspot(
    ...
    if emit_edges:
        try:
            edges_meta = emit_hotspot_edges(
                local_corr=local_corr,
                local_corr_z=local_corr_z,
                local_corr_c=local_corr_c,
                outdir=outdir,
                n_obs=int(adata.n_obs),
                edge_fdr=float(edge_fdr),
                top_n=int(edge_top_n),
            )
        except Exception as exc:
            eprint(f"[WARN] edges_all.tsv write failed: {exc}")
            # Always write empty edges_all.tsv + top file so downstream schema checks pass.
            edges_meta = emit_hotspot_edges(
                local_corr=None,
                local_corr_z=None,
                local_corr_c=None,
        ...
        # Even when edges emission is disabled, still create an empty edges_all.tsv
        # (and the companion top file) with the canonical schema.
        edges_meta = emit_hotspot_edges(
            local_corr=None,
            local_corr_z=None,
            local_corr_c=None,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Emit full gg matrices (0/1, default 1)",
    )
